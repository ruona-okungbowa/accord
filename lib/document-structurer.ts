/**
 * Document structuring utilities for Accord
 * Converts raw extracted text into a structured format optimized for redlining
...
export interface DocumentSection {
  ...
  metadata?: {
    ...
    clauseNumber?: string; // For clauses (e.g., "1.1", "2.3.4")
 ...
 */
export function structureDocumentText(
  rawText: string,
  fileName: string,
  fileType: string
): StructuredDocument {
  ...
  // Split text into paragraphs (double newline or single newline for short lines)
  ...
  for (const paragraph of paragraphs) {
    ...
    currentPosition = endPosition + 2; // Account for paragraph break
    sectionOrder++;
  }
