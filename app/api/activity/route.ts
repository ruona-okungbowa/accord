...
export async function GET(request: NextRequest) {
  try {
    ...
    const { data: issues } = await supabase
      .from("issues")
      .select(
        "id, title, created_at, created_by:users!issues_created_by_fkey(first_name, last_name)"
      )
      .eq("deal_id", dealId)
    ...
    const { data: proposals } = await supabase
      .from("proposals")
      .select(
        "id, title, status, created_at, proposed_by:users!proposals_proposed_by_fkey(first_name, last_name)"
      )
      .eq("deal_id", dealId)
      .order("created_at", { ascending: false });
    ...
    const { data: participants } = await supabase
      ...
      .select(
        "id, accepted_at, user_id:users!deal_participants_user_id_fkey(first_name, last_name), role"
      )
      .eq("deal_id", dealId)
    ...
    const activities = [
      ...
      ...(proposals || []).map((proposal: any) => ({
        ...
        title:
          proposal.status === "accepted"
            ? `Amendment accepted: ${proposal.title}`
            : proposal.status === "rejected"
            ? `Amendment rejected: ${proposal.title}`
            : `Amendment proposed: ${proposal.title}`,
